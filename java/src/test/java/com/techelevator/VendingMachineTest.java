...
public class VendingMachineTest {
	...
	public void testVendingMachine(){
		...
		Assert.assertEquals("A1: Test ($1.00) 1\n", vendor.getProductList());
		Assert.assertEquals("\n", vendor.endPurchase());
		vendor.addBalance(100);
		Assert.assertEquals("Here's your change:\n4 quarters\n\n", vendor.endPurchase());
		Assert.assertEquals("Invalid selection. Please choose a valid slot.", vendor.purchase("A2"));
		Assert.assertEquals("Insufficient funds. Please insert more money.", vendor.purchase("A1"));
		vendor.addBalance(100);
		vendor.purchase("A1");
		...
		Assert.assertEquals("A1: Test ($1.00) SOLD OUT\n", vendor.getProductList());
	}
}
